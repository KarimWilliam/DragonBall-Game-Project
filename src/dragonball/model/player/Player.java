...
public class Player implements Serializable {
	...
	public void removeSuperAttack(SuperAttack SA)
	{
		for(int i = 0; i<this.getActiveFighter().getSuperAttacks().size();i++)
		{
			if(SA.getName().equals(this.getActiveFighter().getSuperAttacks().get(i).getName()))
					{
						this.getActiveFighter().getSuperAttacks().remove(i);
					}
		}
	}
	
	public void removeUltimateAttack(UltimateAttack UA)
	{
		for(int i = 0; i<this.getActiveFighter().getUltimateAttacks().size();i++)
		{
			if(UA.getName().equals(this.getActiveFighter().getUltimateAttacks().get(i).getName()))
					{
						this.getActiveFighter().getUltimateAttacks().remove(i);
					}
		}
	}
